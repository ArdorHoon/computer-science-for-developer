...
ConcurrentMapCacheManager 사용 시, 캐시 값이 임의로 변경되는 경우가 있다. ConcurrentMapCacheManager는 기본적으로 캐시 저장 시, 실제 객체가 아닌 참조 값을 저장하는 특성이 있다. 그렇기 때문에 객체를 저장 시, 객체를 직렬화 하지 않아도 된다. 그렇기 때문에 아래와 같이 Config를 설정해준다.

</br>

* TransactionAwareCacheManagerProxy는 캐시를 트랜잭션 내에서 처리되도록 하기 위한 것
...
이를 테스트 코드를 만들어 실행해보면 아래와 같이 결과가 나오는데 연산 없이 캐싱된 데이터를 반환하는 것을 볼 수 있다.
