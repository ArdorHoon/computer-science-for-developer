...
<mark>**SQL문을 실행하는 가장 효율적인 방법을 결정하는 DBMS의 핵심 엔진**</mark></br>
...
**미리 정의된 규칙을 사용하여 실행 계획을 선택하는 오래된 방식**, 이러한 규칙은 특정 종류의 쿼리나 연산을 우선시하는 경향이 있다.

</br>

### ⭕ 비용 기반 옵티마이저(CBO : Cost-Based Optimizer)
최신 데이터베이스에서 가장 일반적인 방식으로, **다양한 실행 계획의 비용을 계산하고 가장 적은 비용을 가진 계획을 선택** 


CBO는 RBO보다 더 유동적이고, 데이터베이스의 성능에 더 최적화된 방식이다.
...
|인덱스 |인덱스 존재 시 가장 우선 사용 | Cost에 의한 결정 |
...
4. <mark>**비용 모델의 한계**</mark> : 대부분의 옵티마이저는 CBO를 사용하여 실행 계획을 선택하는데 비용 모델이 실제 시스템 환경과 완전히 일치하지 않을 수 있기 때문에, 옵티마이저가 예측한 비용가 실제 실행 비용 간에 차이가 발생할 수 있다.
